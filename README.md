...
1. <b>Infrastructure (Infrastructure Layer)</b>: is where database/JPA in some DDD approaches is located, file system, or any external web service we depend on live <u>-mainly external RestClients</u>.
2. <b>Tests (Test Layer)</b>: unit, integration and end-to-end (how we validate our business cases).
3. <b>User Interface (Presentation Layer></b>: is how the User interacts with the code we have built (probably via a Web Application using HTTP Rest or calling another transportation mechanism of the Application Service Layer) <u>-<code>@Controller</code>, Views, and other components related to the presentation are here</u>.
4. <b>Application Services (Application Layer)</b>: The 3 initial layers are the ones that interact with our "application core", i.e. the <b>Application Services</b> layer a.k.a. the Transport Layer. Within this layer we define what our services can do through a series of contract, e.g. BookInformation <u>-<b>Use Cases/<code>@Service</code></b>, application <b>contracts</b> and application logic services</u>.
5. <b>Domain Services (Domain Layer)</b>: is where the majority of the business logic exists, it carries out the operations to turn A into B, input into output, egg into chicken. It achieves this through interacting with the final layer <u>-<code>@Service</code>, business logic and adapters of DM</u>. ***Note these Services exists in the Domain Layer and sometimes can be omitted if business rules and verifications are added inside DOs as a simplified approach***
6. <b>Domain Model (Domain Layer)</b>: is the representation of the high level data domain objects we use <u>-<code>@Entity, Records and Repositories</code> that behave as aggregates and collections having business rules and verifications</u>.
...
<b>Key Features</b>: 
<u>a) Domain Event Management</u>: <code>AbstractAggregateRoot</code> simplifies the publishing of domain events through an internal event mechanism, i.e. you can register an event within the aggregate by calling <code>registerEvent(Object event)</code> and then publish all registered events at the appropriate time using <code>DomainEventPublisher</code>.
<u>b)Transparent Persistence</u>: Registered events are typically published when the aggregate is saved (persisted) using a Spring Data repository, i.e. Spring Data detects the inheritance from <code>AbstractAggregateRoot</code> and handles the automatic publication of the registered events.
<u>c) Cleaner Code</u>: By extending <code>AbstractAggregateRoot</b>, you reduce the boilerplate code needed to handle events within an aggregate, promoting a clear separation between domain logic and infrastructure details related to event publication.
- 